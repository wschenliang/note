...
2.软链接：软链接不占⽤磁盘空间，源⽂件删除则软链接失效。

```
ln -s 源⽂件 链接⽂件
```

3.硬链接(hard link, 也称链接)：就是⽂件的⼀个或多个⽂件名
...
**注意：**
如果软链接⽂件和源⽂件不在同⼀个⽬录，源⽂件要使⽤绝对路径，不能使⽤相对路径。

4.软连接和硬链接的区别:

- 软链接可以跨⽂件系统，硬链接不可以;
- 软链接可以对⼀个不存在的⽂件名进⾏链接(当然此时如果你vi这个软链接⽂件，linux会⾃动新建⼀个⽂件名为filename的⽂件),硬链接不可以(其⽂件必须存在，inode必须存在);
- 软链接可以对⽬录进⾏连接，硬链接不可以。
